...
void RectangleShape::init(glm::vec3 s, glm::vec3 e) {
  ...
  _v.at(0) = s;
  _v.at(1) = glm::vec3(s.x - ::sin(angle) * sin(a) * l,
                       s.y + l * sin(angle) * cos(a), s.z);

  _v.at(2) = e;
  _v.at(3) = glm::vec3(e.x + ::sin(angle) * sin(a) * l,
                       e.y - l * sin(angle) * cos(a), e.z);

  glBindBuffer(GL_ARRAY_BUFFER, _vbo);
  glBufferSubData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 0,
                  sizeof(glm::vec3) * _v.size(), &_v[0]);
}

void RectangleShape::setStart(glm::vec3 start) { init(start, start); }

void RectangleShape::setEnd(glm::vec3 end) { init(_v.at(0), end); }

void RectangleShape::clear() { init(glm::vec3(), glm::vec3()); }

Points RectangleShape::getPoints() const { return _v; }
