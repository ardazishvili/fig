...
class Animation {
 public:
  ...
  Animation() = delete;
  Animation(BonesData& boneInfos, BoneMapping& boneMapping,
 ...
 private:
  void readNodes(float time, const aiNode* node,
                 const glm::mat4& parentTransform);
  void calcInterpolatedScaling(aiVector3D& Out, float time,
                               const aiNodeAnim* node);
  void calcInterpolatedRotation(aiQuaternion& Out, float time,
                                const aiNodeAnim* node);
  void calcInterpolatedPosition(aiVector3D& Out, float time,
                                const aiNodeAnim* node);
