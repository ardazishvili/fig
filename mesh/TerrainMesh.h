...
class TerrainMesh : public Mesh {
 public:
  virtual void render();
  void init(float bottomLeftX, float bottomLeftY, float topRightX,
            float topRightY, int divisions, float zScale, TerrainType type);
  virtual void calculateHeights(unsigned int width, float bottomLeftX,
                                float bottomLeftY) = 0;
  virtual void calculateColors(unsigned int width,
 ...
 protected:
  virtual void calculateIndices(int divisionsX, int divisionsY,
                                unsigned int latticeWidth) = 0;
