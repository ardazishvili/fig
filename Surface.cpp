...
namespace fig {
Surface::Surface(Camera& camera, Shader& shader, float bottomLeftX,
                 float bottomLeftY, float topRightX, float topRightY,
                 int divisions, std::string texturePath)
    : _camera(camera),
      _shader(shader),
      _bottomLeft(bottomLeftX, bottomLeftY),
      _topRight(topRightX, topRightY) {
  _mesh.initSurface(0, 0, topRightX - bottomLeftX, topRightY - bottomLeftY,
                    divisions, texturePath);
...
void Surface::setOffsetZ(float offset) { _offset.z = offset; }
...
void Surface::setScaleX(float factor) { _xScaleFactor = factor; }
