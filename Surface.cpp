...
Surface::Surface(Camera& camera,
  ...
  _camera(camera),
  _shader(shader), _bottomLeft(bottomLeftX, bottomLeftY), _topRight(topRightX, topRightY)
{
  _mesh.initSurface(0, 0, topRightX - bottomLeftX, topRightY - bottomLeftY, divisions, texturePath);
...
void Surface::render()
{
  ...
  model = glm::rotate(model, glm::radians(_yawAngle), glm::vec3(0.0, 0.0f, 1.0f));
  model = glm::rotate(model, glm::radians(_pitchAngle), glm::vec3(1.0, 0.0f, 0.0f));
